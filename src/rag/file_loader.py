...
class PDFLoader(BaseLoader):
    ...
    def __call__(self, pdf_files:List[str], **kwargs):
        num_processes = min(self.num_processing, kwargs["workers"])
...
class TextSplitter:
    def __init__(self, separators: List[str] = ['\n\n', '\n', ' ', ''],chunk_size: int = 300, chunk_overlap: int = 0) -> None:
        self.spliter = RecursiveCharacterTextSplitter(separators=separators, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
    def __call__(self, documents):
        return self.spliter.split_documents(documents=documents)
